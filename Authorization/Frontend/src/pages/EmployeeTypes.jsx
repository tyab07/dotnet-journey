// ─── EmployeeTypes page ───────────────────────────────────────────────────────
//
// This page shows a table of employee types (e.g. Full-Time, Contract, Intern)
// and lets Admin/SuperAdmin:
//   - View all types
//   - Add a new type
...
function EmployeeTypes() {
  ...
  const loadEmployeeTypes = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || "Failed to load employee types.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (type) => {
    if (!window.confirm(`Delete employee type "${type.name}"?`)) return;
    try {
      await deleteEmployeeType(type.id);
      loadEmployeeTypes();
    } catch (err) {
  ...
  if (loading) return <p className="text-gray-500">Loading employee types…</p>;
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-semibold text-gray-800">
          {empTypes.length} Employee Type{empTypes.length !== 1 ? "s" : ""}
        </h2>
        {!isEmployee && (
          <button
            onClick={openAdd}
                ...
                <tr key={t.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  {!isEmployee && (
                    <td className="px-4 py-3">
                      <button onClick={() => openEdit(t)} className="text-blue-600 hover:underline mr-3 text-xs">Edit</button>
      ...
      <Modal
        isOpen={modalOpen}
        title={isEditing ? "Edit Employee Type" : "Add Employee Type"}
